...
//////////////////////////////////////////////////////////////
    class Virtualizor_Admin_API {
        ...
        function addvs($post=[], $cookies=[]){
            ...
            return [
                'title' => $ret['title'],
                'error' => isset($ret["error"]) && $ret["error"] ? $ret['error'] : [],
                'vs_info' => isset($ret['newvs']) ? $ret['newvs'] : [],
                'globals' => isset($ret['globals']) ? $ret['globals'] : [],
            ];
        }

        function addvs2($post=[], $cookies=[]){
            $path = 'index.php?act=addvs';
            $post = $this->clean_post($post);
            $ret = $this->call($path, '', $post, $cookies);
            return $ret;
        ...
        function servergroups($sg_name = ''){
            $path = 'index.php?act=servergroups';
            return $this->call($path,[],['sg_name' => $sg_name]);
        }
        function listservers($page = 1, $reslen = 50){
            $path = 'index.php?act=servers&page='.$page.'&reslen='.$reslen;
            return $this->call($path);
        }
        function listdnsplans($page = 1, $reslen = 50){
            $path = 'index.php?act=dnsplans&page='.$page.'&reslen='.$reslen;
            return $this->call($path);
        }
        ...
        function mediagroups($page = 1, $reslen = 50){
            $path = 'index.php?act=mediagroups&page='.$page.'&reslen='.$reslen;
            $ret = $this->call($path);
            return $ret;
        }
        ...
        function status($vids){
            // Make the Request
            return $this->call('index.php?act=vs&vs_status='.implode(',', $vids));
        }
        ...
        function vnc2($post=[]){
            $path = 'index.php?act=vnc&launch=1&giveapplet=1';
            $res = $this->call($path, array(), $post);
            return $res;
        }
