<?php
    ...
    { 
        ...
        abstract public function GetCostPrices($parameters);
    ...
    // CURL CONNECTION
    class APIConnection_CURL extends APIConnection
    {
        // VARIABLES
        ...
        function APIConnection_SOAP($Service_URL)
        {
            ...
            // Create unique connection
            $this->service = false;#new SoapClient($this->URL_SERVICE . "?singlewsdl", array("cache_wsdl" => WSDL_CACHE_MEMORY,  "encoding"=>"UTF-8"));
        }
                    ...
                    case "ChildNameServerInfo":
                        {
                            ...
                            if(is_array($attrValue))
                            {
                                ...
                                if(isset($attrValue["ChildNameServerInfo"]["IpAddress"]))
                                {
                                    ...
                                    // Name of NameServer
                                    if(isset($attribute["NameServer"]) && !is_null($attribute["NameServer"]) && is_string($attribute["NameServer"]))
        ...
        private function parseContactInfo($data)
        {
            ...
            return [
                'result' => "ERROR",
                'error'  => "SoapClient class not found"
            ];
        }
        ...
        public function GetCostPrices($parameters)
        {
    ...
    class APIConnection_SOAP extends APIConnection
    {
        ...
        function __construct($Service_URL)
        {
            ...
            $this->service = new SoapClient($this->URL_SERVICE . "?singlewsdl", array("cache_wsdl" => WSDL_CACHE_MEMORY,  "encoding"=>"UTF-8"));
                ...
                $this->service = new error_soap_class();
                    ...
                    if($TldSayisi>1){
                            ...
                            if(is_array($data["DomainAvailabilityInfoList"]["DomainAvailabilityInfo"])) {
                                ...
                                foreach($data["DomainAvailabilityInfoList"]["DomainAvailabilityInfo"] as $name => $value) {
                                    array_push($result["data"],
                                        array(  "DomainName" => $value["DomainName"],//Domain ad�
                                                "TLD" => $value["Tld"], // Uzant�
                                                "Status" => $value["Status"], // Domain al�nabilirlik durumu available notavailable
                                                "IsFee" => $value["IsFee"], // Domain Premium mu True /False
                                                "Currency" => $value["Currency"],// Doviz kuru
                                                "Command" => $value["Command"], // Komut create,renew,transfer,restore fiyatlar�n�n �ekilmesi
                                                "Period" => $value["Period"], // Domain Periyodu
                                                "Price" => $value["Price"], // Domain Fiyat�
                                                "ClassKey" => $value["ClassKey"], //Premium domainin a��klamas�
                                                "Reason" => $value["Reason"] //Domain ile ilgili �zel durumlar rezerve dilmi�  veya al�nam�yor gibi
                                        )
                                    );
                                }
                        ...
                        if(isset($data["DomainAvailabilityInfoList"]["DomainAvailabilityInfo"])) {
                            if(is_array($data["DomainAvailabilityInfoList"]["DomainAvailabilityInfo"])) {
                                ...
                                array_push($result["data"],
                                    array(  "DomainName" => $data["DomainAvailabilityInfoList"]["DomainAvailabilityInfo"]["DomainName"],//Domain ad�
                                            "TLD" => $data["DomainAvailabilityInfoList"]["DomainAvailabilityInfo"]["Tld"], // Uzant�
                                            "Status" => $data["DomainAvailabilityInfoList"]["DomainAvailabilityInfo"]["Status"], // Domain al�nabilirlik durumu available notavailable
                                            ...
                                            "Command" => $data["DomainAvailabilityInfoList"]["DomainAvailabilityInfo"]["Command"], // Komut create,renew,transfer,restore fiyatlar�n�n �ekilmesi
                                            "Period" => $data["DomainAvailabilityInfoList"]["DomainAvailabilityInfo"]["Period"], // Domain Periyodu
                                            "Price" => $data["DomainAvailabilityInfoList"]["DomainAvailabilityInfo"]["Price"], // Domain Fiyat�
                                            "ClassKey" => $data["DomainAvailabilityInfoList"]["DomainAvailabilityInfo"]["ClassKey"], //Premium domainin a��klamas�
                                            "Reason" => $data["DomainAvailabilityInfoList"]["DomainAvailabilityInfo"]["Reason"] //Domain ile ilgili �zel durumlar rezerve dilmi�  veya al�nam�yor gibi
                                    )
                                );
        ...
        public function GetCostPrices($parameters)
        {
            ...
            {
                // SOAP method which is same as current function name called
                $response = $this->service->__soapCall('GetTldList', array($parameters));
    ...
    class DomainNameAPI_PHPLibrary
    {
        ...
        // Get domain cost prices
        public function GetCostPrices()
        {
            $parameters = array(
                "request" => array(
                    "IncludePriceDefinitions" =>true,
                    "OrderColumn" => "Id",
                    "OrderDirection" => "ASC",
                    "Password" => $this->_USERDATA_PASSWORD,
                    "UserName" => $this->_USERDATA_USERNAME,
            ...
            // Get domain id via already prepared connection
            $response = $this->con->GetCostPrices($parameters);
